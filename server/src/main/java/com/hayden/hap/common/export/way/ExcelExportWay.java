 ...
 * @date 2017年6月28日
 */
public class ExcelExportWay extends AbstractExportWay{
	/**
	 * Logger for this class
	 */
	private static final Logger logger = LoggerFactory.getLogger(ExcelExportWay.class);
	...
	private ReturnResult<InputStream> export(List<? extends AbstractVO> voList,FormParamVO formParamVO, String funcnama, Long tenantid, String exportids, int exportcountb, int exportcounte) throws HDException{
		...
		while (it.hasNext()){
			...
			for(int i=exportcountb;i<=endMarker;i++){//行循环
				...
				if (ElementTypeEnum.FILE.getCode().equals(formItemVO.getFitem_input_element())||ElementTypeEnum.IMG.getCode().equals(formItemVO.getFitem_input_element())
						||ElementTypeEnum.PHOTO.getCode().equals(formItemVO.getFitem_input_element())) {
					value = voList.get(i-1).get(formItemVO.getFitem_code()+"_attachShowList");
				}
				if(null != value){
					...
					if (ElementTypeEnum.FAST_IMG.getCode().equals(formItemVO.getFitem_input_element())||ElementTypeEnum.FAST_FILE.getCode().equals(formItemVO.getFitem_input_element())||
							ElementTypeEnum.FILE.getCode().equals(formItemVO.getFitem_input_element())||ElementTypeEnum.IMG.getCode().equals(formItemVO.getFitem_input_element())
							||ElementTypeEnum.PHOTO.getCode().equals(formItemVO.getFitem_input_element())) {
						 row.setHeight((short) 800);
						 if (StringUtils.isEmpty(pk)) {
						 	TableDefVO tableDefVO = tableDefService.queryDetailedTableByTbname(funcService.getOperaTableNameOfFunc(func_code,tenantid));//操作表信息
						 	pk = tableDefVO.getPkColumnVO()!=null?tableDefVO.getPkColumnVO().getColcode():null;
						 }
						 handleImage(formItemVO,voList.get(i-1),wb, patriarch,colIndex,rownum,func_code,pk);
					}else {
